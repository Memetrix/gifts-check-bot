...
from telethon.tl.types import InputUser, ChatBannedRights
...
DELAY = 1.5               # пауза между итерациями очереди
CLICK_COOLDOWN = 10       # анти-спам кликов
...
# ─────────── TeleBot ───────────
bot = TeleBot(bot_token, num_threads=1)   # один поток → без 409
        ...
        try:
            ...
            if username:
                try:
                    ...
                    log.debug("не найден по username: %s", e1)
...
async def process_queue():
    while True:
        ...
        try:
            ...
            # генерируем ссылку-JoinRequest на 1 час
...
def join_req(req):
    """
    req.from_user, req.chat
    Выполняем проверку и approve / decline.
    """
